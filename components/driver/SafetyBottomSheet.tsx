...
const SafetyBottomSheet = ({ onClose, onNavigate }: SafetyBottomSheetProps) => (
  <View className="p-4">
    ...
    <View className="space-y-3">
      <TouchableOpacity onPress={() => onNavigate('/(driver)/safety')} className="bg-black/40 rounded-xl px-4 py-3 flex-row items-center">
      ...
      <TouchableOpacity onPress={() => onNavigate('/(driver)/safety')} className="bg-black/40 rounded-xl px-4 py-3 flex-row items-center">
        <Text className="text-xl mr-3">📍</Text>
        <Text className="text-white font-JakartaBold flex-1">Compartir Viaje</Text>
      </TouchableOpacity>
      
      <TouchableOpacity onPress={() => onNavigate('/(driver)/safety')} className="bg-black/40 rounded-xl px-4 py-3 flex-row items-center">
        <Text className="text-xl mr-3">📞</Text>
        <Text className="text-white font-JakartaBold flex-1">Contactos Emergencia</Text>
      </TouchableOpacity>
      
      <TouchableOpacity onPress={() => onNavigate('/(driver)/safety')} className="bg-black/40 rounded-xl px-4 py-3 flex-row items-center">
        <Text className="text-xl mr-3">📋</Text>
        <Text className="text-white font-JakartaBold flex-1">Reportar Incidente</Text>
      </TouchableOpacity>
    </View>
  </View>
);
