...
const variantMap: Record<FieldVariant, string> = {
  default:
    "bg-white dark:bg-brand-primary rounded-full border border-neutral-200 dark:border-brand-primaryDark",
  ...
  underline:
    "bg-transparent border-b border-neutral-300 dark:border-brand-primaryDark rounded-none",
};
...
export const TextField: React.FC<TextFieldProps> = ({
  ...
  return (
    <View className={`w-full ${containerClassName}`}>
      ...
      <View
        className={`flex-row items-center ${variantMap[variant]} ${hasError ? "border-red-500" : ""}`}
      >
