...
const DriverConfirmationStep: React.FC = () => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      console.log("[DriverConfirmationStep] Driver confirmed, waiting for acceptance");
  ...
  const handleFindAnother = async () => {
    ...
    try {
      ...
      const response = await driverMatchingService.findAnotherDriver(matchingRequest);

      if (response.success && response.driver) {
        console.log("[DriverConfirmationStep] Another driver found:", response.driver);
        ...
        showError("Sin conductores disponibles", "No se encontraron más conductores disponibles");
      ...
      console.error("[DriverConfirmationStep] Error finding another driver:", error);
