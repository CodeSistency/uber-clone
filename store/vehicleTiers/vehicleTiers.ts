...
export const useVehicleTiersStore = create<VehicleTiersStore>((set, get) => ({
  ...
  setTiers: async (tiers: VehicleTiersData) => {
    try {
      ...
      const hasValidTiers = (tiers.car && tiers.car.length > 0) || (tiers.motorcycle && tiers.motorcycle.length > 0);
      ...
      if (hasValidTiers) {
        ...
        console.log('[VehicleTiersStore] No valid tiers to save, skipping AsyncStorage');
      }
  ...
  loadTiersFromStorage: async () => {
    try {
      ...
      console.error('[VehicleTiersStore] Error loading tiers from storage:', error);
  ...
  fetchTiers: async () => {
    try {
      ...
      console.log('[VehicleTiersStore] Response has data property:', !!data.data);
      ...
      if (data && data.data && data.data.data) {
        ...
        console.log('[VehicleTiersStore] Raw tiers keys:', Object.keys(rawTiers));
        ...
        console.error('[VehicleTiersStore] Invalid response structure - no data.data.data');
        console.error('[VehicleTiersStore] Full response:', data);
        console.error('[VehicleTiersStore] Response data keys:', data?.data ? Object.keys(data.data) : 'no data');
  ...
  forceRefreshTiers: async () => {
    try {
      console.log('[VehicleTiersStore] Force refreshing tiers - clearing cache and fetching from API');
      await AsyncStorage.removeItem(STORAGE_KEY);
