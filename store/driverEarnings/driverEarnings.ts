...
export const useDriverEarningsStore = create<DriverEarningsState>((set, get) => ({
  ...
  fetchEarningsSummary: async (period = "today") => {
    const state = get();
    console.log("[DriverEarningsStore] fetchEarningsSummary called for period:", period);
    ...
    try {
      ...
      const totalEarnings = Object.values(summary).reduce((sum, period) => sum + period.earnings, 0);
      const totalTrips = Object.values(summary).reduce((sum, period) => sum + period.rides, 0);
      ...
      console.log("[DriverEarningsStore] Earnings summary fetched successfully");
    } catch (error: any) {
      console.error("[DriverEarningsStore] Error fetching earnings summary:", error);
  ...
  fetchTripHistory: async (limit = 50, offset = 0) => {
    const state = get();
    console.log("[DriverEarningsStore] fetchTripHistory called:", { limit, offset });
    ...
    try {
      ...
      console.log("[DriverEarningsStore] Trip history fetched successfully:", trips.length);
    } catch (error: any) {
      console.error("[DriverEarningsStore] Error fetching trip history:", error);
  ...
  fetchPromotions: async () => {
    ...
    try {
      ...
      console.log("[DriverEarningsStore] Promotions fetched successfully:", promotions.length);
    } catch (error: any) {
      console.error("[DriverEarningsStore] Error fetching promotions:", error);
  ...
  fetchChallenges: async () => {
    ...
    try {
      ...
      console.log("[DriverEarningsStore] Challenges fetched successfully:", challenges.length);
    } catch (error: any) {
      console.error("[DriverEarningsStore] Error fetching challenges:", error);
  ...
  setDateRange: (startDate, endDate) => {
    console.log("[DriverEarningsStore] setDateRange called:", { startDate, endDate });
    set({
      dateRange: {
        startDate,
        endDate,
  ...
  refreshEarnings: async () => {
    ...
    try {
      ...
      console.error("[DriverEarningsStore] Error refreshing earnings:", error);
