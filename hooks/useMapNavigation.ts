...
export const useMapNavigation = () => {
  ...
  const startNavigation = useCallback(async ({ destination, rideId }: StartNavParams) => {
    if (!userLatitude || !userLongitude) return;
    const origin: LatLng = { latitude: userLatitude, longitude: userLongitude };
    ...
    controller.fitBounds(route.polyline, { top: 60, right: 60, bottom: 60, left: 60 } as any);
    ...
    // Start WS updates
    const effectiveRideId = rideId ?? (activeRide as any)?.ride_id ?? (activeRide as any)?.orderId ?? (activeRide as any)?.id ?? null;
    rideIdRef.current = effectiveRideId;
    if (effectiveRideId) {
      try { locationTrackingService.start(effectiveRideId); } catch {}
      ...
      intervalRef.current = setInterval(() => {
        ...
        websocketService.updateDriverLocation(effectiveRideId, { latitude: lat, longitude: lng, timestamp: new Date() } as any);
  ...
  const stopNavigation = useCallback(() => {
    ...
    try { locationTrackingService.stop(); } catch {}
  ...
  const updateDestination = useCallback(async (destination: LatLng) => {
    if (!userLatitude || !userLongitude) return;
    const origin: LatLng = { latitude: userLatitude, longitude: userLongitude };
  ...
  const recenter = useCallback(() => {
    if (userLatitude && userLongitude) {
      controller.panTo({ latitude: userLatitude, longitude: userLongitude }, 600);
    }
  }, [userLatitude, userLongitude, controller]);
