...
async function runTests() {
  ...
  if (!basicTest.result.success && !namespacedTest.result.success) {
    console.log(
      "❌ No WebSocket connection possible - server may not be running",
    );
    ...
    console.log(
      "⚠️  Basic connection works, but /uber-realtime namespace fails",
    );
    console.log(
      "   Server may not have the /uber-realtime namespace configured",
    );
  } else if (!basicTest.result.success && namespacedTest.result.success) {
    ...
    console.log(
      "   Issue may be with authentication or client-side configuration",
    );
  }
