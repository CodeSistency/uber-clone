...
export class EarningsService {
  ...
  async getEarningsSummary(): Promise<EarningsSummary> {
    try {
      ...
      console.error('[EarningsService] Error fetching earnings summary:', error);
      throw error;
    }
  }

  // Trip History
  async getTripHistory(period: 'today' | 'week' | 'month' | 'all' = 'week'): Promise<TripEarning[]> {
    try {
      console.log('[EarningsService] Fetching trip history for period:', period);
      const response = await fetchAPI(`driver/earnings/trips?period=${period}`, {
        requiresAuth: true
      });
      return response;
    } catch (error) {
  ...
  async getActivePromotions(): Promise<Promotion[]> {
    try {
      ...
      console.error('[EarningsService] Error fetching active promotions:', error);
      throw error;
    }
  }
  ...
  async getPromotionDetails(promotionId: string): Promise<Promotion> {
    try {
      ...
      const response = await fetchAPI(`driver/earnings/promotions/${promotionId}`, {
        requiresAuth: true
      });
      return response;
    } catch (error) {
      console.error('[EarningsService] Error fetching promotion details:', error);
      throw error;
    }
  }
  ...
  async getActiveChallenges(): Promise<Challenge[]> {
    try {
      ...
      console.error('[EarningsService] Error fetching active challenges:', error);
      throw error;
    }
  }
  ...
  async getChallengeDetails(challengeId: string): Promise<Challenge> {
    try {
      ...
      const response = await fetchAPI(`driver/earnings/challenges/${challengeId}`, {
        requiresAuth: true
      });
      return response;
    } catch (error) {
      console.error('[EarningsService] Error fetching challenge details:', error);
      throw error;
    }
  }
  ...
  async addPaymentMethod(paymentMethod: Omit<PaymentMethod, 'id' | 'isVerified'>): Promise<PaymentMethod> {
    try {
  ...
  async updatePaymentMethod(paymentMethodId: string, updates: Partial<PaymentMethod>): Promise<PaymentMethod> {
    try {
      console.log('[EarningsService] Updating payment method:', paymentMethodId, updates);
      const response = await fetchAPI(`driver/earnings/payment-methods/${paymentMethodId}`, {
        method: 'PUT',
  ...
  async deletePaymentMethod(paymentMethodId: string): Promise<void> {
    try {
      console.log('[EarningsService] Deleting payment method:', paymentMethodId);
  ...
  // Instant Pay
  async requestInstantPay(request: InstantPayRequest): Promise<InstantPayResult> {
    try {
  ...
  async getInstantPayHistory(): Promise<InstantPayResult[]> {
    try {
      ...
      console.error('[EarningsService] Error fetching instant pay history:', error);
      throw error;
    }
  }

  // Analytics
  async getEarningsAnalytics(period: 'week' | 'month' | 'quarter' | 'year' = 'month'): Promise<{
    hourlyEarnings: Array<{ hour: number; earnings: number }>;
    ...
    topEarningHours: Array<{ hour: number; earnings: number; rides: number }>;
    topEarningDays: Array<{ day: string; earnings: number; rides: number }>;
  }> {
    try {
      console.log('[EarningsService] Fetching earnings analytics for period:', period);
      const response = await fetchAPI(`driver/earnings/analytics?period=${period}`, {
        requiresAuth: true
      });
      return response;
    } catch (error) {
      console.error('[EarningsService] Error fetching earnings analytics:', error);
      throw error;
    }
  }
