...
const initializeFirebaseManually = async () => {
  try {
    console.log('[FirebaseService] Attempting manual Firebase initialization...');
    ...
    if (Platform.OS === 'android') {
      console.log('[FirebaseService] Android detected - relying on google-services.json');
      console.log('[FirebaseService] FCM tokens should work in production builds');
    }
    ...
    console.warn('[FirebaseService] Manual Firebase initialization failed:', error);
    return false;
  }
};
...
export class FirebaseService {
  ...
  async requestPermissions(): Promise<boolean> {
    try {
      ...
      console.log('[FirebaseService] Notification permissions granted:', granted);
  ...
  private async setupNotificationChannel(): Promise<void> {
    if (Platform.OS === 'android') {
      try {
        ...
        console.log('[FirebaseService] Android notification channel configured');
      } catch (error) {
        console.error('[FirebaseService] Error setting up notification channel:', error);
      }
    }
  }
  ...
  async getFCMToken(): Promise<string | null> {
    try {
      ...
      if (this.isRetrying) {
        console.warn('[FirebaseService] Already retrying, skipping to avoid infinite loop');
        return null;
      }
      ...
      if (!isUuid(projectIdCandidate)) {
        console.warn('[FirebaseService] Skipping Expo push token: missing EAS projectId (UUID).');
        return null;
      }

      const tokenData = await Notifications.getExpoPushTokenAsync({ projectId: projectIdCandidate });
      ...
      console.warn('[FirebaseService] 📱 FCM tokens will work in production builds');
      console.warn('[FirebaseService] 🏗️  Build with: npx expo run:android --variant release');
   ...
   */
  async getDeviceInfo(): Promise<{ deviceType: 'ios' | 'android' | 'web'; deviceId: string }> {
    try {
      ...
      if (!deviceId) {
        ...
        deviceId = Device.deviceName || Device.modelName || `device_${Date.now()}_${Math.random().toString(36).substring(2, 15)}`;
  ...
  async getFirebaseTokenData(): Promise<FirebaseTokenData | null> {
    try {
      ...
      console.error('[FirebaseService] Error getting Firebase token data:', error);

      // Log specific Firebase initialization errors
      if (error.message && error.message.includes('FirebaseApp is not initialized')) {
        console.warn('[FirebaseService] Firebase not initialized - this is expected in development. FCM tokens will work in production builds.');
      }
...
export const getFirebaseTokenData = () => firebaseService.getFirebaseTokenData();
