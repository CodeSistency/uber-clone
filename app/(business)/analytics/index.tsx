      ...
      <ScrollView className="flex-1 px-5">
          ...
          {[
            ...
            <TouchableOpacity
              ...
              className={`flex-1 py-2 px-4 rounded-md ${
                selectedPeriod === period.key ? 'bg-primary-500' : 'bg-transparent'
              }`}
            >
              <Text className={`text-center font-JakartaBold ${
                selectedPeriod === period.key ? 'text-white' : 'text-secondary-600'
              }`}>
                {period.label}
              </Text>
            </TouchableOpacity>
        ...
        <View className="bg-white rounded-lg p-4 mb-4">
          ...
          <View className="bg-general-500 rounded-lg p-3">
            <Text className="text-sm font-JakartaBold mb-2">Performance vs Last Period</Text>
            <View className="flex-row justify-between">
              <Text className="text-sm text-secondary-600">Revenue</Text>
              <Text className="text-sm font-JakartaBold text-success-500">+12.5%</Text>
            </View>
            <View className="flex-row justify-between mt-1">
              <Text className="text-sm text-secondary-600">Orders</Text>
              <Text className="text-sm font-JakartaBold text-success-500">+8.3%</Text>
            </View>
            <View className="flex-row justify-between mt-1">
              <Text className="text-sm text-secondary-600">Customers</Text>
              <Text className="text-sm font-JakartaBold text-success-500">+15.2%</Text>
            </View>
          </View>
        </View>
        ...
        <View className="bg-white rounded-lg p-4 mb-4">
          <Text className="text-lg font-JakartaBold mb-4">Order Distribution by Hour</Text>

          <View className="space-y-2">
            {DUMMY_HOURLY_DATA.map((hourData) => (
              <View key={hourData.hour} className="flex-row items-center">
                <Text className="w-12 text-sm font-JakartaMedium">{hourData.hour}</Text>
                <View className="flex-1 bg-general-500 rounded-full h-3 mx-3">
                  <View
                    className="bg-primary-500 h-3 rounded-full"
                    style={{ width: `${(hourData.orders / maxHourlyOrders) * 100}%` }}
                  />
                </View>
        ...
        <View className="bg-white rounded-lg p-4 mb-4">
          ...
          {DUMMY_POPULAR_ITEMS.map((item, index) => (
            <View key={item.name} className="flex-row items-center py-3 border-b border-general-500 last:border-b-0">
              ...
              <View className="items-end">
                <Text className={`text-sm font-JakartaBold ${
                  item.trend.startsWith('+') ? 'text-success-500' : 'text-danger-500'
                }`}>
                  {item.trend}
                </Text>
              </View>
        ...
        {/* Customer Insights */}
        <View className="bg-white rounded-lg p-4 mb-4">
          <Text className="text-lg font-JakartaBold mb-4">Customer Insights</Text>
          ...
          <View className="space-y-4">
            <View className="flex-row justify-between items-center">
              ...
              <Text className="text-xl font-JakartaExtraBold text-success-500">24</Text>
            </View>

            <View className="flex-row justify-between items-center">
              <View>
                <Text className="font-JakartaBold">Repeat Customers</Text>
                <Text className="text-sm text-secondary-600">Returning visitors</Text>
              </View>
              <Text className="text-xl font-JakartaExtraBold text-primary-500">19</Text>
            </View>

            <View className="flex-row justify-between items-center">
              <View>
                <Text className="font-JakartaBold">Average Rating</Text>
                <Text className="text-sm text-secondary-600">From customer reviews</Text>
              </View>
              <View className="flex-row items-center">
                <Text className="text-xl font-JakartaExtraBold text-warning-500 mr-1">4.7</Text>
