...
import { safetyService, EmergencyContact, EmergencyAlert, ShareTripData, IncidentReport } from "@/app/services/safetyService";
...
const SafetyScreen = () => {
  ...
  const [contacts, setContacts] = useState<EmergencyContact[]>(dummyEmergencyContacts);
  const [alerts, setAlerts] = useState<EmergencyAlert[]>(dummyActiveAlerts);
  const [shareData, setShareData] = useState<ShareTripData | null>(dummyShareTripData);
  const [incidents, setIncidents] = useState<IncidentReport[]>(dummyIncidentReports);
  ...
  const handleEmergencyAlert = (type: 'sos' | 'accident' | 'medical' | 'other') => {
    Alert.alert(
      "Emergency Alert",
      ...
      [
        ...
        { 
          ...
          onPress: () => {
            ...
            Alert.alert("Alert Sent", "Emergency services have been notified and your contacts will be alerted.");
          }
        }
      ]
    );
      ...
      Alert.alert(
        ...
        [
          ...
          { 
            ...
            onPress: () => {
              setShareData(null);
              Alert.alert("Trip Sharing Stopped", "Your location is no longer being shared.");
            }
          }
        ]
      );
  ...
  const renderEmergencyButton = () => (
    <View className="mx-4 mb-4">
      ...
      >
        ...
        <Text className="text-white font-JakartaBold text-xl">Emergency SOS</Text>
    ...
    <View className="mx-4 mb-4">
      ...
      <View className="flex-row space-x-3">
        ...
        >
          ...
          <Text className="text-white font-JakartaBold text-sm">Report Accident</Text>
        </TouchableOpacity>
        ...
        >
          <Text className="text-white text-2xl mb-1">🏥</Text>
          <Text className="text-white font-JakartaBold text-sm">Medical Emergency</Text>
        </TouchableOpacity>
        ...
        >
          <Text className="text-white text-2xl mb-1">📋</Text>
          <Text className="text-white font-JakartaBold text-sm">Report Incident</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
  ...
  const renderEmergencyContacts = () => (
    <View className="mx-4 mb-4">
      <View className="flex-row items-center justify-between mb-3">
        <Text className="text-lg font-JakartaBold text-black dark:text-white">Emergency Contacts</Text>
        <TouchableOpacity onPress={() => router.push('/(driver)/safety/contacts' as any)}>
          <Text className="text-brand-primary font-JakartaBold">Manage</Text>
        </TouchableOpacity>
      </View>
      
      {contacts.map((contact) => (
        <View key={contact.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 mb-3 shadow-sm">
          <View className="flex-row items-center justify-between">
            <View className="flex-1">
              <View className="flex-row items-center mb-1">
                <Text className="font-JakartaBold text-black dark:text-white">{contact.name}</Text>
                {contact.isPrimary && (
              ...
              <Text className="text-gray-600 dark:text-gray-400 text-sm">{contact.phone}</Text>
              <Text className="text-gray-500 dark:text-gray-500 text-xs">{contact.relationship}</Text>
            </View>
            ...
            <View className="items-center">
              ...
              <Text className="text-xs text-gray-500 dark:text-gray-500">Call</Text>
            </View>
          </View>
        </View>
      ))}
  ...
  const renderTripSharing = () => (
    <View className="mx-4 mb-4">
      <View className="flex-row items-center justify-between mb-3">
        <Text className="text-lg font-JakartaBold text-black dark:text-white">Trip Sharing</Text>
      ...
      {shareData ? (
        <View className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm">
          <View className="flex-row items-center mb-2">
            ...
            <Text className="font-JakartaBold text-black dark:text-white">Trip Sharing Active</Text>
          </View>
  ...
  const renderActiveAlerts = () => {
    ...
    return (
      <View className="mx-4 mb-4">
        <Text className="text-lg font-JakartaBold text-black dark:text-white mb-3">Active Alerts</Text>
        
        {alerts.map((alert) => (
          <View key={alert.id} className="bg-red-100 dark:bg-red-900 rounded-xl p-4 mb-3 shadow-sm">
  ...
  const renderIncidentReports = () => (
    <View className="mx-4 mb-4">
      <View className="flex-row items-center justify-between mb-3">
        <Text className="text-lg font-JakartaBold text-black dark:text-white">Recent Reports</Text>
        <TouchableOpacity onPress={() => router.push('/(driver)/safety/incidents' as any)}>
          <Text className="text-brand-primary font-JakartaBold">View All</Text>
        </TouchableOpacity>
      </View>
      
      {incidents.map((incident) => (
        <View key={incident.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 mb-3 shadow-sm">
          <View className="flex-row items-center justify-between mb-2">
  ...
  const renderSafetySettings = () => (
    <View className="mx-4 mb-4">
      <Text className="text-lg font-JakartaBold text-black dark:text-white mb-3">Safety Settings</Text>
      ...
      <View className="space-y-3">
        ...
        >
          <View className="flex-row items-center">
            ...
            <Text className="font-JakartaBold text-black dark:text-white">Safety Preferences</Text>
          </View>
          <Text className="text-gray-400">›</Text>
        </TouchableOpacity>
        
        <TouchableOpacity 
          onPress={() => router.push('/(driver)/safety/emergency-services' as any)}
          className="bg-white dark:bg-gray-800 rounded-xl p-4 flex-row items-center justify-between shadow-sm"
        >
          <View className="flex-row items-center">
            <Text className="text-2xl mr-3">🚨</Text>
            <Text className="font-JakartaBold text-black dark:text-white">Emergency Services</Text>
          </View>
          <Text className="text-gray-400">›</Text>
        </TouchableOpacity>
        ...
        >
          <View className="flex-row items-center">
            <Text className="text-2xl mr-3">✅</Text>
            <Text className="font-JakartaBold text-black dark:text-white">Safety Check</Text>
          </View>
          <Text className="text-gray-400">›</Text>
        </TouchableOpacity>
      </View>
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50 dark:bg-gray-900">
      ...
      <View className="flex-row items-center justify-between px-4 py-3 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        ...
        <Text className="text-lg font-JakartaBold text-black dark:text-white">Safety Toolkit</Text>
        <TouchableOpacity onPress={() => router.push('/(driver)/safety/settings' as any)}>
