      ...
      <ScrollView className="flex-1 px-5">
        ...
        <View className="bg-white rounded-lg p-6 mb-4">
          ...
          <View className="bg-general-500 rounded-lg p-3">
            <View className="flex-row justify-between">
              ...
              <Text className="font-JakartaBold">${(currentData.total - currentData.tips).toFixed(2)}</Text>
            </View>
            <View className="flex-row justify-between mt-2">
              <Text className="font-JakartaMedium">Tips</Text>
              <Text className="font-JakartaBold text-success-500">+${currentData.tips.toFixed(2)}</Text>
            </View>
          </View>
        </View>
        ...
        <View className="bg-white rounded-lg p-4 mb-4">
          ...
          {DUMMY_TRIP_HISTORY.map((trip) => (
            <View key={trip.id} className="border-b border-general-500 py-3 last:border-b-0">
              <View className="flex-row justify-between items-center mb-2">
                <Text className="font-JakartaBold">
                  ${trip.fare.toFixed(2)}
                  {trip.tips > 0 && (
                    <Text className="text-success-500"> +${trip.tips.toFixed(2)} tip</Text>
                  )}
                </Text>
              ...
              <View className="flex-row justify-between items-center">
                ...
                <TouchableOpacity className="bg-primary-500 px-3 py-1 rounded-full">
                  <Text className="text-white text-xs font-JakartaBold">Details</Text>
                </TouchableOpacity>
              </View>
            </View>
          ))}
        ...
        <View className="bg-white rounded-lg p-4 mb-4">
          ...
          <View className="bg-general-500 rounded-full h-3 mb-2">
            <View className="bg-success-500 h-3 rounded-full" style={{ width: '75%' }} />
          </View>
          <View className="flex-row justify-between">
            <Text className="text-sm text-secondary-600">$285.25 earned</Text>
            <Text className="text-sm text-secondary-600">$350 goal</Text>
          </View>
          <Text className="text-sm text-success-500 mt-1">75% complete • $64.75 to go</Text>
        </View>
        ...
        <View className="bg-white rounded-lg p-4 mb-8">
          ...
          <View className="flex-row justify-between items-center">
            <View>
              ...
              <Text className="text-sm text-secondary-600">Available to withdraw</Text>
            </View>
