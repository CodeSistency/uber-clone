...
import { ratingsService, Rating, RatingSummary, PerformanceMetrics, Feedback } from "@/app/services/ratingsService";
...
const RatingsScreen = () => {
  ...
  const [ratingSummary, setRatingSummary] = useState<RatingSummary>(dummyRatingSummary);
  const [localRecentRatings, setLocalRecentRatings] = useState<Rating[]>(dummyRecentRatings);
  const [metrics, setMetrics] = useState<PerformanceMetrics>(dummyPerformanceMetrics);
      ...
      <View className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
        ...
        <View className="flex-row justify-between">
          <View className="items-center">
            ...
            <Text className="text-gray-600 dark:text-gray-400 text-sm">This Week</Text>
          </View>
          <View className="items-center">
            <Text className="text-2xl font-JakartaBold text-blue-600">
              {ratingSummary.averageRatingLastMonth}
            </Text>
            <Text className="text-gray-600 dark:text-gray-400 text-sm">This Month</Text>
          </View>
          <View className="items-center">
            <Text className={`text-2xl font-JakartaBold ${
              ratingSummary.recentTrend === 'up' ? 'text-green-600' : 
              ratingSummary.recentTrend === 'down' ? 'text-red-600' : 'text-gray-600'
            }`}>
              {ratingSummary.recentTrend === 'up' ? '↗️' : 
               ratingSummary.recentTrend === 'down' ? '↘️' : '→'}
            </Text>
  ...
  const renderRatingDistribution = () => (
    <View className="mx-4 mb-4">
      <Text className="text-lg font-JakartaBold text-black dark:text-white mb-3">Rating Distribution</Text>
      
      <View className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm">
        {[5, 4, 3, 2, 1].map((stars) => {
          const starKey = stars === 5 ? 'five' : stars === 4 ? 'four' : stars === 3 ? 'three' : stars === 2 ? 'two' : 'one';
          const count = ratingSummary.ratingDistribution[starKey as keyof typeof ratingSummary.ratingDistribution] ?? 0;
          ...
          return (
            <View key={stars} className="flex-row items-center mb-2">
              <View className="flex-row items-center w-16">
                <Text className="text-gray-600 dark:text-gray-400 text-sm mr-1">{stars}</Text>
  ...
  const renderCategoryScores = () => (
    <View className="mx-4 mb-4">
      <Text className="text-lg font-JakartaBold text-black dark:text-white mb-3">Category Scores</Text>
      
      <View className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm">
        {Object.entries(ratingSummary.categoryAverages).map(([category, score]) => (
          <View key={category} className="flex-row items-center justify-between mb-3 last:mb-0">
            <Text className="font-JakartaBold text-black dark:text-white capitalize">
              {category}
            </Text>
            <View className="flex-row items-center">
              <Text className="text-gray-600 dark:text-gray-400 mr-2">{score}</Text>
  ...
  const renderPerformanceMetrics = () => (
    <View className="mx-4 mb-4">
      <Text className="text-lg font-JakartaBold text-black dark:text-white mb-3">Performance Metrics</Text>
      ...
      <View className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm">
        <View className="flex-row justify-between mb-3">
          <View className="items-center">
            ...
            <Text className="text-gray-600 dark:text-gray-400 text-sm">Acceptance</Text>
          </View>
          <View className="items-center">
            <Text className="text-2xl font-JakartaBold text-blue-600">
              {metrics.completionRate}%
            </Text>
            <Text className="text-gray-600 dark:text-gray-400 text-sm">Completion</Text>
          </View>
          <View className="items-center">
            <Text className="text-2xl font-JakartaBold text-purple-600">
              {metrics.onTimeRate}%
            </Text>
            <Text className="text-gray-600 dark:text-gray-400 text-sm">On Time</Text>
          </View>
        </View>
        ...
        <View className="flex-row justify-between">
          <View className="items-center">
            ...
            <Text className="text-gray-600 dark:text-gray-400 text-sm">Response</Text>
          </View>
          <View className="items-center">
            <Text className="text-2xl font-JakartaBold text-red-600">
              {metrics.cancellationRate}%
            </Text>
            <Text className="text-gray-600 dark:text-gray-400 text-sm">Cancellation</Text>
          </View>
          <View className="items-center">
            <Text className="text-2xl font-JakartaBold text-indigo-600">
              {metrics.customerSatisfaction}
            </Text>
            <Text className="text-gray-600 dark:text-gray-400 text-sm">Satisfaction</Text>
          </View>
        </View>
      </View>
    </View>
  ...
  const renderRecentRatings = () => (
    <View className="mx-4 mb-4">
      <View className="flex-row items-center justify-between mb-3">
        <Text className="text-lg font-JakartaBold text-black dark:text-white">Recent Ratings</Text>
        <TouchableOpacity onPress={() => router.push('/(driver)/ratings/history' as any)}>
          <Text className="text-brand-primary font-JakartaBold">View All</Text>
        </TouchableOpacity>
      </View>
      
      {localRecentRatings.map((rating) => (
        <View key={rating.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 mb-3 shadow-sm">
          <View className="flex-row items-center justify-between mb-2">
            <Text className="font-JakartaBold text-black dark:text-white">{rating.passengerName}</Text>
            <View className="flex-row items-center">
              <Text className="text-gray-600 dark:text-gray-400 mr-1">{rating.rating}</Text>
              <Text className="text-yellow-500">⭐</Text>
            </View>
          </View>
          ...
          <View className="flex-row flex-wrap mb-2">
            {rating.tags.map((tag, index) => (
              <View key={index} className="bg-blue-100 dark:bg-blue-900 px-2 py-1 rounded-full mr-2 mb-1">
  ...
  const renderFeedback = () => (
    <View className="mx-4 mb-4">
      <View className="flex-row items-center justify-between mb-3">
        <Text className="text-lg font-JakartaBold text-black dark:text-white">Recent Feedback</Text>
        <TouchableOpacity onPress={() => router.push('/(driver)/ratings/feedback' as any)}>
          <Text className="text-brand-primary font-JakartaBold">View All</Text>
        </TouchableOpacity>
      </View>
      
      {feedback.map((item) => (
        <View key={item.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 mb-3 shadow-sm">
          <View className="flex-row items-center justify-between mb-2">
            <Text className="font-JakartaBold text-black dark:text-white">{item.passengerName}</Text>
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50 dark:bg-gray-900">
      ...
      <View className="flex-row items-center justify-between px-4 py-3 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        ...
        <Text className="text-lg font-JakartaBold text-black dark:text-white">Ratings & Performance</Text>
        <TouchableOpacity onPress={() => router.push('/(driver)/ratings/settings' as any)}>
