...
export default function CompletionSuccess() {
  ...
  useEffect(() => {
    const t = setTimeout(() => router.replace('/(root)/(tabs)/home' as any), 1200);
  ...
  return (
    <SafeAreaView className={`flex-1 bg-brand-primary dark:bg-brand-primaryDark items-center justify-center`}>
      <View className="items-center">
        ...
        <Text className={`text-2xl font-JakartaBold text-black dark:text-white mb-1`}>All set!</Text>
        <Text className={`text-gray-600 dark:text-gray-300`}>Taking you to the app…</Text>
      </View>
    </SafeAreaView>
  );
}
