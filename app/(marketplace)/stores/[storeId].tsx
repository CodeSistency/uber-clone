...
const DUMMY_STORE = {
  ...
  image: '🍕',
  isOpen: true,
  description: 'Authentic Italian pizza made with fresh ingredients and traditional recipes. Family-owned and operated for over 20 years.',
...
const StoreDetails = () => {
  ...
  const filteredItems = DUMMY_MENU_ITEMS.filter(item => item.category === selectedCategory);
  const cartTotal = cart.reduce((total, item) => total + (item.item.price * item.quantity), 0);
  const cartItemCount = cart.reduce((total, item) => total + item.quantity, 0);

  const handleAddToCart = (menuItem: any) => {
    const existingItem = cart.find(cartItem => cartItem.item.id === menuItem.id);
    ...
    <SafeAreaView className="flex-1 bg-general-500">
      ...
      <View className="bg-white p-5">
        ...
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center">
            ...
            <Text className="text-secondary-600">({DUMMY_STORE.reviewCount})</Text>
          </View>
          <Text className="text-secondary-600">{DUMMY_STORE.deliveryTime}</Text>
          <Text className="text-secondary-600">${DUMMY_STORE.deliveryFee} delivery</Text>
        </View>
      </View>
      ...
      <ScrollView className="flex-1">
        ...
        <View className="bg-white mx-5 mt-4 p-4 rounded-lg mb-4">
          ...
          <Text className="text-secondary-600 mb-3">{DUMMY_STORE.description}</Text>
              ...
              {DUMMY_MENU_CATEGORIES.map((category) => (
                <TouchableOpacity
                  ...
                  className={`px-4 py-2 rounded-full ${
                    selectedCategory === category.id ? 'bg-primary-500' : 'bg-general-500'
                  }`}
                >
            ...
            <View key={item.id} className="bg-white rounded-lg p-4 mb-3">
              <View className="flex-row items-start">
                ...
                <View className="flex-1">
                    ...
                    <View className="flex-1 mr-2">
                      ...
                      {item.isPopular && (
                        <View className="bg-warning-500 px-2 py-1 rounded-full self-start mt-1">
                          <Text className="text-white text-xs font-JakartaBold">Popular</Text>
                        </View>
                      )}
                    </View>
                  ...
                  >
                    <Text className="text-white font-JakartaBold">Add to Cart</Text>
                  </TouchableOpacity>
                </View>
              </View>
            </View>
