...
describe("useMapCenter", () => {
  ...
  describe("Basic Functionality", () => {
    test("returns null when user location is not provided", () => {
      const { result } = renderHook(() => useMapCenter(null, null));

      expect(result.current).toBeNull();
    });

    test("returns user location when only user coordinates provided", () => {
      const { result } = renderHook(() => useMapCenter(40.7128, -74.006));
    ...
    test("calculates center between user and destination", () => {
      const { result } = renderHook(() =>
        useMapCenter(40.7128, -74.006, 40.7589, -73.9851),
      );

      expect(result.current).toEqual({
        latitude: (40.7128 + 40.7589) / 2, // Center latitude
        longitude: (-74.006 + -73.9851) / 2, // Center longitude
        latitudeDelta: expect.any(Number),
        longitudeDelta: expect.any(Number),
      });
    });
  ...
  describe("Options Handling", () => {
    ...
    test("respects bottomSheetHeight option", () => {
      const { result: result1 } = renderHook(() =>
        useMapCenter(40.7128, -74.006, undefined, undefined, {
      ...
      expect(result1.current?.latitudeDelta).not.toBe(
        result2.current?.latitudeDelta,
      );
    });
    ...
    test("respects side offset", () => {
      ...
      expect(result1.current?.longitudeDelta).not.toBe(
        result2.current?.longitudeDelta,
      );
    });
  });
    ...
    test("returns same reference for same inputs", () => {
      const { result, rerender } = renderHook(
        (props: { lat: number; lng: number }) =>
          useMapCenter(props.lat, props.lng),
  ...
  describe("Edge Cases", () => {
    test("handles zero coordinates", () => {
      const { result } = renderHook(() => useMapCenter(0, 0));
    ...
    test("handles extreme coordinates", () => {
      const { result } = renderHook(() => useMapCenter(90, 180));
    ...
    test("handles same user and destination coordinates", () => {
      const { result } = renderHook(() =>
        useMapCenter(40.7128, -74.006, 40.7128, -74.006),
      );
  ...
  describe("Performance", () => {
    ...
    test("minimum delta of 0.01", () => {
      const { result } = renderHook(
        () => useMapCenter(40.7128, -74.006, 40.7129, -74.0061), // Very close points
      );
...
describe("useBusinessMapCenter", () => {
  test("uses business-specific configuration", () => {
    const { result } = renderHook(() =>
      useBusinessMapCenter(40.7128, -74.006, 40.7589, -73.9851),
    );
  ...
  test("returns null when business location not provided", () => {
    const { result } = renderHook(() => useBusinessMapCenter(null, null));
...
describe("useDriverMapCenter", () => {
  test("uses driver-specific configuration", () => {
    const { result } = renderHook(() =>
      useDriverMapCenter(40.7128, -74.006, 40.7589, -73.9851),
    );
  ...
  test("falls back to driver location when no pickup", () => {
    const { result } = renderHook(() => useDriverMapCenter(40.7128, -74.006));
