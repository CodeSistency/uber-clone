...
describe('calculateDistance', () => {
  ...
  test('handles null coordinates', () => {
    const result = calculateDistance(null as any, { latitude: 0, longitude: 0 });
...
describe('isValidLocation', () => {
  test('validates correct location objects', () => {
    expect(isValidLocation({ latitude: 40.7128, longitude: -74.0060 })).toBe(true);
  ...
  test('rejects invalid location objects', () => {
    ...
    expect(isValidLocation({ longitude: -74.0060 })).toBe(false);
    expect(isValidLocation({ latitude: 'invalid', longitude: -74.0060 })).toBe(false);
  });
});
