...
describe('Expo Notifications - End-to-End Integration', () => {
  beforeEach(() => {
    ...
    const mockPermissions = { granted: true, canAskAgain: true, status: 'granted' };
    const mockToken = { type: 'expo', data: 'ExponentPushToken[xxxxxxxxxxxxxxxxxxxxxx]' };

    // Mock de Expo Notifications
    jest.mocked(require('expo-notifications').getPermissionsAsync).mockResolvedValue(mockPermissions);
    jest.mocked(require('expo-notifications').requestPermissionsAsync).mockResolvedValue(mockPermissions);
    jest.mocked(require('expo-notifications').getExpoPushTokenAsync).mockResolvedValue(mockToken);
    jest.mocked(require('expo-notifications').scheduleNotificationAsync).mockResolvedValue('notification-id-123');
    jest.mocked(require('expo-notifications').setBadgeCountAsync).mockResolvedValue(undefined);
    jest.mocked(require('expo-notifications').getBadgeCountAsync).mockResolvedValue(0);

    // Mock del config
    jest.mocked(require('@/lib/expo-notifications/config').expoNotificationConfig.getConfig).mockReturnValue({
    ...
    jest.mocked(require('@/lib/expo-notifications/config').expoNotificationConfig.setupExpoNotificationHandler).mockReturnValue(undefined);
    jest.mocked(require('@/lib/expo-notifications/config').expoNotificationConfig.setupExpoAndroidChannels).mockResolvedValue(undefined);

    // Mock del token manager
    jest.mocked(require('@/lib/expo-notifications/tokenManager').expoTokenManager.getPushToken).mockResolvedValue(mockToken);
    jest.mocked(require('@/lib/expo-notifications/tokenManager').expoTokenManager.clearTokens).mockResolvedValue(undefined);
  });
  ...
  describe('Complete User Journey', () => {
    it('should handle complete notification lifecycle', async () => {
      ...
      await waitFor(() => {
        ...
        expect(result.current.notifications[0].message).toBe('Thanks for using our app');
    ...
    it('should handle permissions workflow', async () => {
      ...
      const deniedPermissions = { granted: false, canAskAgain: true, status: 'denied' };
      const grantedPermissions = { granted: true, canAskAgain: true, status: 'granted' };

      jest.mocked(require('expo-notifications').getPermissionsAsync).mockResolvedValue(deniedPermissions);
      jest.mocked(require('expo-notifications').requestPermissionsAsync).mockResolvedValue(grantedPermissions);
    ...
    it('should handle token management', async () => {
      const mockToken = { type: 'expo', data: 'ExponentPushToken[yyyyyyyyyyyyyyyyyyyyyy]' };
      jest.mocked(require('expo-notifications').getExpoPushTokenAsync).mockResolvedValue(mockToken);
      jest.mocked(require('@/lib/expo-notifications/tokenManager').expoTokenManager.getPushToken).mockResolvedValue(mockToken);
    ...
    it('should handle badge management', async () => {
      jest.mocked(require('expo-notifications').getBadgeCountAsync).mockResolvedValue(3);
  ...
  describe('Error Handling', () => {
    it('should handle notification service errors gracefully', async () => {
      ...
      jest.mocked(require('expo-notifications').scheduleNotificationAsync).mockRejectedValue(
    ...
    it('should handle initialization errors', async () => {
      ...
      jest.mocked(require('expo-notifications').requestPermissionsAsync).mockRejectedValue(
