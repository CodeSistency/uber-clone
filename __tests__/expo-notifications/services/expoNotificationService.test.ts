...
describe('ExpoNotificationService', () => {
  ...
  describe('initialize()', () => {
    it('should initialize service successfully on first call', async () => {
      ...
      const mockGetPushToken = jest.fn().mockResolvedValue({ type: 'expo', data: 'token123' });
      const mockRequestPermissions = jest.fn().mockResolvedValue({ granted: true, status: 'granted' });

      jest.mocked(require('@/lib/expo-notifications/tokenManager').expoTokenManager.getPushToken).mockImplementation(mockGetPushToken);
    ...
    it('should handle permission denied gracefully', async () => {
      const mockRequestPermissions = jest.fn().mockResolvedValue({ granted: false, status: 'denied' });

      jest.mocked(Notifications.requestPermissionsAsync).mockImplementation(mockRequestPermissions);
    ...
    it('should setup Android channels on Android platform', async () => {
      ...
      jest.mocked(require('@/lib/expo-notifications/config').expoNotificationConfig.setupExpoAndroidChannels).mockImplementation(mockSetupAndroidChannels);
  ...
  describe('sendLocalNotification()', () => {
    ...
    it('should send local notification successfully', async () => {
      ...
      jest.mocked(Notifications.scheduleNotificationAsync).mockImplementation(mockSchedule);
    ...
    it('should add notification to store', async () => {
      ...
      jest.mocked(Notifications.scheduleNotificationAsync).mockImplementation(mockSchedule);
      jest.mocked(require('@/store/expo-notifications/expoNotificationStore').useExpoNotificationStore.getState).mockReturnValue(mockStore);
  ...
  describe('Permissions', () => {
    describe('requestPermissions()', () => {
      it('should request permissions and update store', async () => {
        const mockPermissions = { granted: true, status: 'granted', canAskAgain: true };
        const mockSetPermissions = jest.fn();

        jest.mocked(Notifications.getPermissionsAsync).mockResolvedValue({ status: 'undetermined' });
        jest.mocked(Notifications.requestPermissionsAsync).mockResolvedValue(mockPermissions);
        jest.mocked(require('@/store/expo-notifications/expoNotificationStore').useExpoNotificationStore.getState).mockReturnValue({
    ...
    describe('checkPermissions()', () => {
      it('should check current permissions', async () => {
        ...
        jest.mocked(Notifications.getPermissionsAsync).mockResolvedValue(mockPermissions);
  ...
  describe('Badge Management', () => {
    describe('setBadgeCount()', () => {
      it('should set badge count', async () => {
        ...
        jest.mocked(Notifications.setBadgeCountAsync).mockImplementation(mockSetBadge);
  ...
  describe('Android Channels', () => {
    ...
    describe('createChannel()', () => {
      it('should create notification channel on Android', async () => {
        ...
        jest.mocked(Notifications.setNotificationChannelAsync).mockImplementation(mockSetChannel);
    ...
    describe('deleteChannel()', () => {
      it('should delete notification channel on Android', async () => {
        ...
        jest.mocked(Notifications.deleteNotificationChannelAsync).mockImplementation(mockDeleteChannel);
    ...
    describe('getChannels()', () => {
      it('should get notification channels on Android', async () => {
        const mockChannels = [{ id: 'test', name: 'Test' }];
        jest.mocked(Notifications.getNotificationChannelsAsync).mockResolvedValue(mockChannels);
  ...
  describe('Event Management', () => {
    describe('addEventListener()', () => {
      it('should add event listener and return listener ID', () => {
        ...
        const listenerId = expoNotificationService.addEventListener('notificationReceived', mockCallback);
    ...
    describe('removeEventListener()', () => {
      ...
      it('should return false for non-existent listener', () => {
        const removed = expoNotificationService.removeEventListener('non-existent');
    ...
    describe('removeAllListeners()', () => {
      it('should remove all listeners for specific event', () => {
        expoNotificationService.addEventListener('notificationReceived', jest.fn());
        expoNotificationService.addEventListener('notificationReceived', jest.fn());
        expoNotificationService.addEventListener('notificationResponse', jest.fn());
  ...
  describe('Utility Methods', () => {
    describe('getNotificationContent()', () => {
      it('should return correct content for RIDE_REQUEST', () => {
        const content = expoNotificationService.getNotificationContent('RIDE_REQUEST', {
      ...
      it('should return correct content for RIDE_ACCEPTED', () => {
        const content = expoNotificationService.getNotificationContent('RIDE_ACCEPTED', {
          driverName: 'John'
        });
      ...
      it('should return default content for unknown types', () => {
        const content = expoNotificationService.getNotificationContent('UNKNOWN_TYPE' as any);
    ...
    describe('simulateNotification()', () => {
      it('should simulate notification reception', () => {
        const mockHandleNotification = jest.spyOn(expoNotificationService as any, 'handleNotificationReceived');
  ...
  describe('destroy()', () => {
    it('should cleanup resources', async () => {
      ...
      jest.mocked(require('@/lib/expo-notifications/tokenManager').expoTokenManager.clearTokens).mockImplementation(mockClearTokens);
