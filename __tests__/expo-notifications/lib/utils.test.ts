...
describe("Notification Utils", () => {
  ...
  describe("getPriorityFromType()", () => {
    ...
    it("should return normal for unknown types", () => {
      expect(getPriorityFromType("UNKNOWN_TYPE" as ExpoNotificationType)).toBe(
        "normal",
      );
    });
  });
  ...
  describe("shouldShowNotification()", () => {
    ...
    it("should return true for enabled notification types", () => {
      expect(shouldShowNotification(basePreferences, "RIDE_REQUEST")).toBe(
        true,
      );
      expect(shouldShowNotification(basePreferences, "EMERGENCY_ALERT")).toBe(
        true,
      );
    });
    ...
    it("should hide non-critical notifications during quiet hours", () => {
      ...
      expect(
        shouldShowNotification(preferences, "RIDE_REQUEST", quietHour),
      ).toBe(false);
      expect(
        shouldShowNotification(preferences, "EMERGENCY_ALERT", quietHour),
      ).toBe(true);
    });
  });
  ...
  describe("getNotificationColor()", () => {
    ...
    it("should return default color for unknown types", () => {
      expect(getNotificationColor("UNKNOWN_TYPE" as ExpoNotificationType)).toBe(
        "#6B7280",
      );
    });
  });
