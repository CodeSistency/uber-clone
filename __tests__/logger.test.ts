...
describe('Logger', () => {
  ...
  describe('Log Filtering', () => {
    ...
    it('should filter by log level', () => {
      ...
      const testWarnLogs = warnLogs.filter(log => log.message.includes('Message'));
      expect(testWarnLogs.length).toBe(1);
      expect(testWarnLogs[0].message).toBe('Message B1');

      const errorLogs = logger.getLogs(LogLevel.ERROR);
      const testErrorLogs = errorLogs.filter(log => log.message.includes('Message'));
    ...
    it('should filter by category', () => {
      ...
      const testCategoryALogs = categoryALogs.filter(log => log.message.includes('Message'));
      expect(testCategoryALogs.length).toBe(3);

      const categoryBLogs = logger.getLogs(undefined, 'CategoryB');
      const testCategoryBLogs = categoryBLogs.filter(log => log.message.includes('Message'));
    ...
    it('should filter by both level and category', () => {
      ...
      const testFilteredLogs = filteredLogs.filter(log => log.message.includes('Message'));
      expect(testFilteredLogs.length).toBe(2);
      expect(testFilteredLogs.every(log => log.category === 'CategoryA')).toBe(true);
      expect(testFilteredLogs.every(log => log.level === LogLevel.INFO)).toBe(true);
    });
  ...
  describe('Performance Logging', () => {
    it('should log performance metrics', () => {
      ...
      logger.performance('TestService', 'testOperation', startTime, { param: 'value' });
    ...
    it('should warn for slow operations', () => {
      ...
      const logs = logger.getLogs(LogLevel.WARN);
      expect(logs.some(log => log.message.includes('Performance'))).toBe(true);
    });
  });
  ...
  describe('Network Logging', () => {
    ...
    it('should log failed network requests as errors', () => {
      logger.network('APIService', 'POST', '/api/users', 500, 200, new Error('Server error'));
  ...
  describe('WebSocket Logging', () => {
    ...
    it('should log WebSocket errors', () => {
      ...
      logger.websocket('WebSocketService', 'connection_error', { attempt: 1 }, wsError);
  ...
  describe('User Action Logging', () => {
    it('should log user actions', () => {
      logger.userAction('UI', 'button_pressed', { buttonId: 'submit', screen: 'login' });
  ...
  describe('Log Management', () => {
    it('should clear logs', () => {
      ...
      const userLogsAfterClear = logsAfterClear.filter(log => log.message.includes('Test message'));
  ...
  describe('Log Level Filtering', () => {
    it('should respect log level settings', () => {
      ...
      const logs = logger.getLogs();
      const errorLogs = logs.filter(log => log.message.includes('Error message'));
