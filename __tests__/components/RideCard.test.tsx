  ...
  describe("Basic Rendering", () => {
    ...
    test("renders different ride statuses", () => {
      const { rerender, getByText } = render(
        <RideCard
          {...defaultProps}
          ride={{ ...mockRide, payment_status: "pending" }}
        />,
      );
      ...
      rerender(
        <RideCard
          {...defaultProps}
          ride={{ ...mockRide, payment_status: "in_progress" }}
        />,
      );
      expect(getByText("123 Main Street, City")).toBeTruthy();
      ...
      rerender(
        <RideCard
          {...defaultProps}
          ride={{ ...mockRide, payment_status: "cancelled" }}
        />,
      );
      expect(getByText("123 Main Street, City")).toBeTruthy();
    });
  ...
  describe("Data Formatting", () => {
    test("formats currency correctly", () => {
      const { getByText } = render(
        <RideCard
          {...defaultProps}
          ride={{ ...mockRide, fare_price: 100.99 }}
        />,
      );
  ...
  describe("Performance", () => {
    test("renders efficiently with large data", () => {
      const largeRideData = {
        ...mockRide,
        origin_address:
          "A very long pickup location name that might cause performance issues if not handled properly",
        destination_address:
          "An extremely long destination address that could potentially slow down rendering if not optimized",
      };
  ...
  describe("Styling", () => {
    ...
    test("handles long text gracefully", () => {
      const longTextRide = {
        ...mockRide,
        origin_address:
          "This is a very long pickup location that should be handled gracefully without breaking the layout",
        destination_address:
          "This is an even longer destination address that needs to be displayed properly",
      };
