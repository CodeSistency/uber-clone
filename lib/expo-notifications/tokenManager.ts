...
import { ExpoPushToken, ExpoTokenData, ExpoNotificationError } from "../../types/expo-notifications";
...
class ExpoTokenManager {
  ...
  async getPushToken(forceRefresh = false): Promise<ExpoPushToken | null> {
    ...
    try {
      ...
      if (!forceRefresh && this.currentToken) {
        ...
        if (isValid) {
          log.info("ExpoTokenManager", "Using cached valid token", { operationId });
      ...
      if (!forceRefresh && cachedToken) {
        ...
        if (isValid) {
          ...
          log.warn("ExpoTokenManager", "Stored token is invalid, requesting new one", {
            operationId
          });
        }
      }
      ...
      log.error("ExpoTokenManager", "Failed to obtain push token", { operationId });
      return null;

    } catch (error) {
      log.error("ExpoTokenManager", "Error getting push token", {
        operationId,
   ...
   */
  private async requestNewToken(operationId: string): Promise<ExpoPushToken | null> {
    try {
      ...
      log.error("ExpoTokenManager", "Failed to request new token", {
        operationId,
        error: (error as Error)?.message,
        platform: Platform.OS,
      }, error instanceof Error ? error : undefined);
  ...
  async validateToken(token: ExpoPushToken): Promise<boolean> {
    try {
      ...
      if (!token.data.startsWith("ExponentPushToken[") && !token.data.startsWith("ExponentPushToken")) {
      ...
      const timestamp = await SecureStore.getItemAsync(this.TOKEN_TIMESTAMP_KEY);
      if (timestamp) {
  ...
  private async saveTokenToStorage(token: ExpoPushToken): Promise<void> {
    try {
      ...
      await Promise.all([
        ...
        SecureStore.setItemAsync(this.TOKEN_DATA_KEY, JSON.stringify(tokenData)),
        SecureStore.setItemAsync(this.TOKEN_TIMESTAMP_KEY, Date.now().toString()),
      ]);
      ...
      log.error("ExpoTokenManager", "Failed to save token to storage", {
        error: (error as Error)?.message,
      }, error instanceof Error ? error : undefined);
      throw error;
    }
...
export const getExpoPushToken = (forceRefresh?: boolean) => expoTokenManager.getPushToken(forceRefresh);
export const refreshExpoToken = () => expoTokenManager.refreshToken();
export const clearExpoTokens = () => expoTokenManager.clearTokens();
export const validateExpoToken = (token: ExpoPushToken) => expoTokenManager.validateToken(token);
